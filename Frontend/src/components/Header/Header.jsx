...
function Header() {
  ...
  const getNotificationMessage = (n) => {
    ...
    switch (n.type) {
      case "video_like":
        return `${sender} liked your video`;
      case "comment":
        return `${sender} commented on your video`;
      case "subscription":
        return `${sender} subscribed to your channel`;
      case "comment_like":
    ...
    <div className='flex flex-col justify-center hidden sm:flex h-full'> 
      ...
      >
        <div className="relative group">
          ...
          />
          <Search
            className="absolute left-4 top-2.5 text-zinc-500 group-focus-within:text-blue-500 transition-colors"
            size={18}
          />
        </div>
      </form>
        ...
        {status ? (
          <div className="flex items-center gap-4 md:gap-6 relative">
            ...
            <button
              ...
              className={`p-2 rounded-xl transition-all hover:bg-white/5 relative ${unreadCount > 0 ? "text-blue-500" : "text-zinc-400"}`}
            >
            ...
            {showNotif && (
              <div className="absolute top-14 right-0 w-[320px] md:w-[380px] bg-[#121212] border border-white/10 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] p-5 z-[110] animate-in fade-in slide-in-from-top-2 duration-300">
                <div className="flex justify-between items-center mb-5">
                  <h3 className="font-black text-white uppercase italic tracking-widest text-xs">
                    Feed
                  </h3>
                  {notifications.length > 0 && (
                    <button
                      onClick={handleClearAll}
                      className="flex items-center gap-1 text-[10px] text-blue-500 font-black uppercase tracking-tighter hover:text-blue-400 transition-colors"
                    >
                ...
                <div className="space-y-2 max-h-[450px] overflow-y-auto no-scrollbar">
                  {notifications.length > 0 ? (
                    notifications.map((n) => (
                      <div
                        key={n._id}
                        className={`group relative p-3 rounded-2xl cursor-pointer flex gap-3 items-center transition-all border border-transparent ${!n.isRead ? "bg-blue-600/5 border-blue-500/20" : "hover:bg-white/5"}`}
                        onClick={() => handleMarkAsRead(n._id)}
                      >
                        <img
                          src={getSecureUrl(n.fromUser?.avatar)}
                          className="w-10 h-10 rounded-full object-cover border border-white/10"
                          alt=""
                        />
                        <div className="flex-1 min-w-0 pr-6">
                          <p
                            className={`${!n.isRead ? "text-white font-bold" : "text-zinc-500"} text-[11px] leading-tight`}
                          >
                            {getNotificationMessage(n)}
                          </p>
                          <span className="text-[9px] text-zinc-600 mt-1 block font-black uppercase tracking-widest">
                        ...
                        <button
                          ...
                          className="opacity-0 group-hover:opacity-100 absolute right-3 text-zinc-600 hover:text-red-500 transition-all"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
            ...
            >
              <img
                ...
                className="w-10 h-10 rounded-2xl object-cover border-2 border-white/5 hover:border-blue-500/50 transition-all shadow-lg"
              />
            </Link>
          ...
          <div className="flex items-center gap-4">
            ...
            <Link
              to="/signup"
              className="bg-blue-600 text-white px-6 py-2.5 rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-blue-500 transition-all shadow-[0_0_20px_rgba(59,130,246,0.3)]"
            >
              Join Now
            </Link>
          </div>
