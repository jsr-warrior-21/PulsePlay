    ...
    return (
        <div className="max-w-3xl mx-auto p-4 text-left min-h-screen text-white">
            ...
            <div className="bg-[#1a1a1a] p-6 rounded-[2.5rem] border border-gray-800 mb-10 shadow-2xl">
                ...
                {/* Selected Image Preview (Padded & Smaller) */}
                {imagePreview && (
                    <div className="relative mt-4 mb-2 bg-[#0f0f0f] p-3 rounded-3xl border border-gray-800 overflow-hidden shadow-inner">
                        ...
                        <button 
                            ...
                            className="absolute top-5 right-5 bg-black/80 text-white p-1 rounded-full text-xs hover:bg-red-600 transition-all border border-white/10"
                        >
                            ✕
                        </button>
                    </div>
                ...
                <div className="flex justify-between items-center pt-4 border-t border-gray-800/50 mt-4">
                    <button onClick={() => fileInputRef.current.click()} className="text-zinc-400 hover:text-blue-500 flex items-center gap-2 transition-all group">
                        <span className="text-xl group-hover:scale-110 transition-transform">🖼️</span>
                        <span className="text-xs font-black uppercase tracking-widest">Add Photo</span>
                        <input type="file" ref={fileInputRef} hidden accept="image/*" onChange={handleImageChange} />
                    </button>
                    <button onClick={handlePostTweet} className="bg-blue-600 hover:bg-blue-500 px-10 py-2.5 rounded-full font-black uppercase text-xs tracking-widest shadow-lg active:scale-95 transition-all">
                        Post
                    </button>
                </div>
            </div>
                    ...
                    <div key={t._id} className="bg-[#1a1a1a] p-6 rounded-[2.5rem] border border-gray-800 group relative shadow-xl hover:border-gray-700 transition-all">
                        <div className="flex gap-5">
                            ...
                            <div className="flex-1">
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    {userData?._id === (t.owner?._id || t.owner) && (
                                        <div className="flex gap-4 opacity-0 group-hover:opacity-100 transition-all duration-300">
                                            <button onClick={() => {setEditingId(t._id); setEditContent(t.content)}} className="text-blue-500 text-[10px] font-black uppercase hover:underline">Edit Text</button>
                                            <button onClick={() => handleDeleteTweet(t._id)} className="text-red-500 text-[10px] font-black uppercase hover:underline">Delete Post</button>
                                        </div>
                                    )}
                                </div>
                                ...
                                {editingId === t._id ? (
                                    <div className="mt-2">
                                        <textarea className="w-full bg-[#0f0f0f] border border-gray-700 p-4 rounded-2xl text-gray-200 outline-none focus:border-blue-500 transition-all" value={editContent} onChange={(e) => setEditContent(e.target.value)} />
                                        <div className="flex gap-2 mt-2">
                                            <button onClick={() => handleUpdateTweet(t._id)} className="bg-blue-600 px-6 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest">Save</button>
                                            <button onClick={() => setEditingId(null)} className="bg-zinc-800 px-6 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest">Cancel</button>
                                        </div>
                                    </div>
                                ) : (
                                    <>
                                            ...
                                            <div className="mt-4 bg-[#0c0c0c] rounded-[2rem] border border-gray-800/50 p-4 group/img relative shadow-inner">
                                                <div className="overflow-hidden rounded-2xl">
                                                    <img 
                                                        ...
                                                        className="w-full max-h-[400px] object-contain mx-auto block transition-transform duration-700 hover:scale-[1.03]" 
                                                        alt="post" 
                                                    />
                                                </div>
                                                ...
                                                {userData?._id === (t.owner?._id || t.owner) && (
                                                    <button 
                                                        ...
                                                        className="absolute top-6 right-6 bg-red-600/90 hover:bg-red-600 text-[8px] font-black uppercase tracking-widest px-4 py-2 rounded-full opacity-0 group-hover/img:opacity-100 transition-all shadow-2xl backdrop-blur-md border border-white/10"
                                                    >
                                                        Remove Photo
                                                    </button>
                                                )}
